...
if __name__ == "__main__":
    ...
    conn.execute(
        "create table if not exists old_posts (id integer primary key, media_id text, code text, like_count int, comment_count int, image_url text, created_at timestamp)"
    )
    ...
    try:
        ...
        if not os.path.isfile(settings_file):
            ...
            # login new
            api = Client(
                username, password, on_login=lambda x: onlogin_callback(x, ".cookies")
            )
        else:
